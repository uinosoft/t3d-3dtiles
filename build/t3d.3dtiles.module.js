// t3d-3dtiles
import { Matrix4, Ray, Vector3, Matrix3, Frustum, Vector2, Box3, Plane, Spherical, Sphere, Loader, TEXTURE_FILTER, TEXTURE_WRAP, Texture2D, BasicMaterial, PBR2Material, TEXEL_ENCODING_TYPE, PBRMaterial, DRAW_SIDE, Buffer, Attribute, Geometry, PointsMaterial, Material, VERTEX_COLOR, SHADING_TYPE, DirectionalLight, PointLight, SpotLight, Vector4, Bone, Object3D, Camera, SkinnedMesh, Mesh, Skeleton, KeyframeClip, QuaternionLinearInterpolant, LinearInterpolant, QuaternionCubicSplineInterpolant, CubicSplineInterpolant, StepInterpolant, VectorKeyframeTrack, NumberKeyframeTrack, QuaternionKeyframeTrack, FileLoader, ImageLoader, DefaultLoadingManager, ShaderLib, MATERIAL_TYPE, Color3, Quaternion, EventDispatcher, LoadingManager } from 't3d';
		...
		return Promise.all(
			gltf.bufferViews.map(bufferView => {
				...
				if (bufferView.extensions) {
					const { EXT_meshopt_compression: EXT_meshopt_compression$1 } = bufferView.extensions;
...
let MaterialParser$2 = class MaterialParser {
	...
	static parse(context) {
		...
		for (let i = 0; i < gltf.materials.length; i++) {
			...
			const { KHR_materials_unlit: KHR_materials_unlit$1, KHR_materials_pbrSpecularGlossiness: KHR_materials_pbrSpecularGlossiness$1, KHR_materials_clearcoat: KHR_materials_clearcoat$1 } = extensions;
	...
	static parse(context, loader) {
		...
		for (let i = 0; i < gltf.meshes.length; i++) {
			...
			for (let j = 0; j < gltfMesh.primitives.length; j++) {
				...
				const { KHR_draco_mesh_compression: KHR_draco_mesh_compression$1 } = extensions;
...
class NodeParser {
	...
	static parse(context) {
		...
		const nodes = gltfNodes.map(gltfNode => {
			...
			const { KHR_lights_punctual: KHR_lights_punctual$1 } = extensions;
...
let MaterialParser$1 = class MaterialParser {
	...
	static parse(context) {
		...
		for (let i = 0; i < gltf.materials.length; i++) {
			...
			const { KHR_materials_unlit: KHR_materials_unlit$1, KHR_materials_pbrSpecularGlossiness: KHR_materials_pbrSpecularGlossiness$1, KHR_materials_clearcoat: KHR_materials_clearcoat$1, KHR_techniques_webgl } = extensions;
...
class MaterialParser {
	...
	static parse(context) {
		...
		for (let i = 0; i < gltf.materials.length; i++) {
			...
			const { KHR_materials_unlit, KHR_materials_pbrSpecularGlossiness: KHR_materials_pbrSpecularGlossiness$1, KHR_materials_clearcoat: KHR_materials_clearcoat$1 } = extensions;
...
export { B3DMLoader, CMPTLoader, LoadParser as DebugLoadParser, I3DMLoader, InstancedBasicMaterial, InstancedPBRMaterial, OBB, PNTSLoader, TileGLTFLoader, Tiles3D };
