...
import { PBRMaterial, TEXEL_ENCODING_TYPE, DRAW_SIDE, Geometry, PointsMaterial, Material, BasicMaterial, VERTEX_COLOR, SHADING_TYPE, Vector3, Quaternion, Matrix4, Attribute, Buffer, Color3, Mesh, Object3D, Ray, Matrix3, Box3, Plane, Spherical, Sphere, Euler, MathUtils, Vector2, Frustum, PIXEL_FORMAT, PIXEL_TYPE, LoadingManager, EventDispatcher, PlaneGeometry, ShaderMaterial, Raycaster, MATERIAL_TYPE, ShaderLib, Texture2D, TEXTURE_FILTER, DefaultLoadingManager, Triangle, LineMaterial, BoxGeometry, DRAW_MODE, Camera } from 't3d';
...
class TilesGroup extends Object3D {
...
const _pos$4 = /* @__PURE__ */ new Vector3();
...
class EnvironmentControls extends EventDispatcher {
	...
	_alignCameraUp(up, alpha = 1) {
		...
		let multiplier = MathUtils.mapLinear(1 - Math.abs(_forward$1.dot(up)), 0, 0.2, 0, 1);
...
class GlobeControls extends EnvironmentControls {
	...
	getPivotPoint(target) {
		...
		if (
			...
			_pos$3.subVectors(target, _ray.origin).dot(_ray.direction) > _pos$3.subVectors(_vec$1, _ray.origin).dot(_ray.direction)
		) {
			target.copy(_vec$1);
		}
