...
import { MathUtils, PlaneGeometry, Sphere, Vector2, Vector3 } from 't3d';
...
export class EllipsoidProjectionTilesPlugin extends ImageFormatPlugin {
	...
	async parseToMesh(buffer, tile, ...args) {
		...
		if (shape === 'ellipsoid') {
			...
			tile.cached.boundingVolume.getBoundingSphere(_sphere);
			for (let i = 0; i < vertCount; i++) {
				_pos.fromArray(position.buffer.array, i * 3);
				_norm.fromArray(normal.buffer.array, i * 3);
				...
				_norm.toArray(normal.buffer.array, i * 3);
			}
